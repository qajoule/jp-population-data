...
async function run() {
  ...
  try {
    ...
    console.log(`rule_contrast: Chromium binary=${command} flags=${args.filter((argument) => argument.startsWith("--")).join(" ")} debugging_endpoint_wait_ms=${debugging.waitedMs} timeout_ms=${debuggingEndpointTimeoutMs}`);
